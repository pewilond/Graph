...
public:
    ...
    virtual TElement &Get(const TKey &key) override;
    virtual const TElement &Get(const TKey &key) const override;
...
private:
    ...
    ShrdPtr<DynamicArraySmart<LinkedListSmart<KeyValuePair>>> table;
...
HashTable<TKey, TElement>::HashTable(size_t initialCapacity)
    : table(new DynamicArraySmart<LinkedListSmart<KeyValuePair>>(static_cast<int>(initialCapacity))),
      count(0),
...
template <typename TKey, typename TElement>
TElement &HashTable<TKey, TElement>::Get(const TKey &key)
{
...
template <typename TKey, typename TElement>
const TElement &HashTable<TKey, TElement>::Get(const TKey &key) const
{
    size_t index = HashFunction(key) % capacity;
...
void HashTable<TKey, TElement>::Rehash()
{
    ...
    ShrdPtr<DynamicArraySmart<LinkedListSmart<KeyValuePair>>> newTable(
...
bool HashTable<TKey, TElement>::HashTableIterator::MoveNext()
{
    ...
    while (bucketIndex < hashTable->capacity)
    {
        ...
        LinkedListSmart<KeyValuePair> &chain =
            hashTable->table->Get(static_cast<int>(bucketIndex));
...
TKey HashTable<TKey, TElement>::HashTableIterator::GetCurrentKey() const
{
    ...
    const LinkedListSmart<KeyValuePair> &chain =
        hashTable->table->Get(static_cast<int>(bucketIndex));

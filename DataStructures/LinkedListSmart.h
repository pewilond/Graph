...
private:
    ...
    static std::shared_ptr<Node> CopyChain(std::shared_ptr<Node> src)
    {
        if (!src)
        {
            return nullptr;
        }
        auto newHead = std::make_shared<Node>(src->data);
        auto currentNew = newHead;
        auto currentOld = src->next;
        while (currentOld)
        {
            currentNew->next = std::make_shared<Node>(currentOld->data);
...
public:
    ...
    LinkedListSmart(const LinkedListSmart &other)
        : head(CopyChain(other.head)), length(other.length) {}
